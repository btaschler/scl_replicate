### 
### GIES - Vary p - (A) shrna / (B) yeast data ###
### 
...
db_vary_p_gies_path = file.path(data_dir, "sampled_data", "vary_p_gies")
...
for (p in p_seq) {
    ...
    for (k in rep_seq) {
        ...
        # Get data (X, G_star)
        data = cad_process_data_gies(graph$obs_data, graph$int_data, graph$int_indices, graph$G_star,
        ...
        # Assign
        db_vary_p_gies[[ sprintf('p_%s__rep_%s', p, k) ]] =
            list(X          = data$X[c(1:19, 19+row_ids), gene_ids],
...
for (p in p_seq) {
    ...
    if (p > max(method_p_list$method_gies)) {
        next
    }
    ...
    for (k in rep_seq) {
        ...
        data = db_vary_p_gies[[ sprintf('p_%s__rep_%s', p, k) ]]
...
# filesstrings::file.move(
#     list.files(file.path("~", "cad_datasets", "vary_p_gies"), full.names = TRUE),
#     file.path(get_lab_folder(), "cad", "datasets", "vary_p", "vary_p_gies")
# )
