...
fci.est <- rfci(suffStat, indepTest = gaussCItest, p = ncol(data), alpha=0.01, rules=rules)
...
lv.ida.est <- lv.ida(x,y,cov(data),fci.est@amat,method="local",localcap = NULL)
print(lv.ida.est) # this is the multiset of causal effects of x = 2 on y = pvar
print(min(abs(lv.ida.est), na.rm=TRUE))